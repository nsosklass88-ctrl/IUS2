...
const IUS_TASKS_SHEET_NAME = 'tasks';
const IUS_TASK_HEADERS = [
  'id', 'order', 'title', 'shortTitle', 'mode', 'prep', 'answer', 'instruction', 'content', 'updatedAt'
];

const IUS_DEFAULT_TASKS = [
  {
    id: 'task-1',
    order: 1,
    ...
    prep: 60,
    answer: 120,
    instruction: 'Тиэкиһи болҕойон аах. Бэлэмнэнии кэмэ — 1 мүнүүтэ. Онтон чуолкайдык, интонациялаахтык ааҕыҥ.',
    content: 'Саха сылгыта – тымныыны тулуйар дьикти боруода.\n\nСаха сылгыта – аан дойдуга саамай тымныыны тулуйар дьиэ кыыла. Бу сылгылар тоҕус ый устата аһаҕас халлаан анныгар сылдьаллар, сиртэн-буортан астарын булуналлар. Кыһын халыҥ хаары туйахтарынан хаһан, хагдарыйбыт оту булан аһыыллар.\n\nСаха сылгыта намыһах уҥуохтаах, түүтэ олус хойуу, сиэлэ-кутуруга уһун. Тириитин анныгар халыҥ сыа мунньуллар, ол тымныыны туоруурга көмөлөһөр.\n\nСылгы – саха омук тыыннаах бэлиэтэ, кинини кытта саха айылҕатын, тулуурун, иһирэх сыһыанын кэрэһэлиир.'
  },
  {
    id: 'task-2',
    order: 2,
    title: '2 сорудах. Тиэкиһи кэпсээһин',
    shortTitle: 'Кэпсээһин',
    mode: 'retell',
    prep: 60,
    answer: 180,
    instruction: 'Аахпыт тиэкискин сиһилии кэпсээ. Бэлэмнэнэр кэмҥэр сүрүн санаалары бэлиэтэн.',
    content: 'Аахпыт тиэкискэ олоҕуран кэпсээҥ.\n\nКэпсээриҥэр умнума:\n- тиэкис сүрүн санаатын;\n- саха сылгыта тоҕо уратылаах буоларын;\n- сылгы саха олоҕор ханнык суолталааҕын;\n- бэйэ санааҕын.'
  },
  {
    id: 'task-3',
    order: 3,
    title: '3 сорудах. Сэһэргээһии',
    shortTitle: 'Сэһэргээһии',
    mode: 'monologue',
    prep: 60,
    answer: 180,
    instruction: 'Сэһэргэһии тиэмэтиттэн биири талан кэпсээҥ. Бэлэмнэнии кэмэ — 1 мүнүүтэ, кэпсээһин — 3 мүнүүтэ.',
    content: 'Тиэмилэр:\n\n1. Хартыынаны ойуулааһын.\n2. Сүрүн боппуруоһу тойоннооһун: Саха тыла — төрүт тыл, кинини харыстыыр эбэтэр хаалларар кэм кэллэ.\n3. Олоххо буолбут түгэнтэн сэһэргээһин (көҥүл тиэмэ).\n\nКэпсииргэр умнума:\n- Саха тыла — ийэ тылым...\n- Мин санаабар, ...\n- Тоҕо диэтэххэ, ...\n- Холобур, ...\n- Түмүккэ, ол иһин...'
  },
  {
    id: 'task-4',
    order: 4,
    ...
    prep: 0,
    answer: 180,
    instruction: 'Ыйытыыларга толору хоруйу биэр. Бэлэмнэнии кэмэ суох, тута эппиэттээ.',
    content: 'Кэпсэтии ыйытыылара:\n\n1. Саха тылын харыстыырга оскуола ханнык көмөнү оҥоруон сөбүй?\n2. Оҕолор төрүт тылларын билэллэригэр дьиэ кэргэн оруола ханнык?\n3. Эн бэйэҥ саха тылын сайыннарыыга туох оҥоруоххун сөбүй?'
  }
];
...
function setupIUS() {
  ...
  const tasksSheet = ensureTasksSheet_();
  const folder = getOrCreateAudioFolder_();
  return 'Готово: лист ' + sheet.getName() + ', лист заданий ' + tasksSheet.getName() + ', папка Drive: ' + folder.getName();
}
...
function doPost(e) {
  try {
    ...
    if (payload.action === 'saveTask') {
      const task = saveTask_(payload.task || {});
      return json_({ ok: true, task });
    }

    if (payload.action === 'resetTasks') {
      resetTasks_();
...
function doGet(e) {
  ...
  try {
    ...
    } else if (p.action === 'tasks') {
      data = { ok: true, tasks: listTasks_() };
    } else {
...
function listTasks_() {
  const sheet = ensureTasksSheet_();
  const values = sheet.getDataRange().getValues();
  if (values.length <= 1) return IUS_DEFAULT_TASKS.slice();
  const tasks = values.slice(1)
    .filter(row => row.some(cell => cell !== ''))
    .map(rowToTask_)
    .filter(task => task && task.id)
    .sort((a, b) => Number(a.order || 0) - Number(b.order || 0));
  return tasks.length ? tasks : IUS_DEFAULT_TASKS.slice();
}

function saveTask_(task) {
  const sheet = ensureTasksSheet_();
  const h = taskHeaderMap_();
  const now = new Date().toISOString();
  const safe = normalizeTask_(task, now);
  const values = sheet.getDataRange().getValues();
  let targetRow = -1;

  for (let r = 1; r < values.length; r++) {
    if (String(values[r][h.id]) === String(safe.id)) {
      targetRow = r + 1;
      break;
    }
  }
  ...
  sheet.getRange(targetRow, 1, 1, IUS_TASK_HEADERS.length).setValues([makeTaskRow_(safe)]);
  SpreadsheetApp.flush();
  return safe;
}

function resetTasks_() {
  const sheet = ensureTasksSheet_();
  if (sheet.getLastRow() > 1) {
    sheet.getRange(2, 1, sheet.getLastRow() - 1, IUS_TASK_HEADERS.length).clearContent();
  }
  const rows = IUS_DEFAULT_TASKS.map(task => makeTaskRow_(normalizeTask_(task, new Date().toISOString())));
  sheet.getRange(2, 1, rows.length, IUS_TASK_HEADERS.length).setValues(rows);
  SpreadsheetApp.flush();
}

function normalizeTask_(task, updatedAt) {
  const order = Math.max(1, Number(task.order || task.id || 1));
  const id = String(task.id || ('task-' + order)).trim();
  return {
    id,
    order,
    title: String(task.title || ('Задание ' + order)).trim(),
    shortTitle: String(task.shortTitle || task.title || ('Задание ' + order)).trim().slice(0, 80),
    mode: String(task.mode || 'monologue').trim(),
    prep: Math.max(0, Number(task.prep || 0)),
    answer: Math.max(30, Number(task.answer || 180)),
    instruction: String(task.instruction || '').slice(0, 45000),
    content: String(task.content || '').slice(0, 45000),
    updatedAt: updatedAt || task.updatedAt || new Date().toISOString()
  };
}

function makeTaskRow_(task) {
  return [
    task.id,
    task.order,
    task.title,
...
function rowToTask_(row) {
  const h = taskHeaderMap_();
  if (!row[h.id]) return null;
  return normalizeTask_({
    id: row[h.id],
    order: row[h.order],
    title: row[h.title],
    shortTitle: row[h.shortTitle],
    mode: row[h.mode],
    prep: row[h.prep],
    answer: row[h.answer],
    instruction: row[h.instruction],
    content: row[h.content],
    updatedAt: row[h.updatedAt]
  }, row[h.updatedAt] || new Date().toISOString());
}

function ensureTasksSheet_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  if (!ss) throw new Error('Откройте Apps Script из Google Таблицы: Расширения → Apps Script');
  let sheet = ss.getSheetByName(IUS_TASKS_SHEET_NAME);
  if (!sheet) sheet = ss.insertSheet(IUS_TASKS_SHEET_NAME);

  if (sheet.getMaxColumns() < IUS_TASK_HEADERS.length) {
    sheet.insertColumnsAfter(sheet.getMaxColumns(), IUS_TASK_HEADERS.length - sheet.getMaxColumns());
  }

  const firstRow = sheet.getRange(1, 1, 1, IUS_TASK_HEADERS.length).getValues()[0];
  const mismatch = IUS_TASK_HEADERS.some((header, index) => firstRow[index] !== header);
  if (mismatch) {
    sheet.getRange(1, 1, 1, IUS_TASK_HEADERS.length).setValues([IUS_TASK_HEADERS]);
  }
...
function taskHeaderMap_() {
  const map = {};
  IUS_TASK_HEADERS.forEach((name, index) => map[name] = index);
  return map;
}
